...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(SPEECHBUBBLE_QUESTTRADER) 10.36 */
	...
	registerMethod("Item", "useInPosition", LuaScriptInterface::luaItemUseInPosition);
	...
	registerMethod("Monster", "executeSpell", LuaScriptInterface::luaMonsterExecuteSpell);
	...
	registerMethod("Npc", "setSpeechBubble", LuaScriptInterface::luaNpcSetSpeechBubble); 10.36 */
...
int32_t LuaScriptInterface::luaItemUseInPosition(lua_State* L)
{
	// item:useInPosition(player, fromPos, toPos)
	Item* item = getUserdata<Item>(L, 1);
	Player* player = getUserdata<Player>(L, 2);
	Position& fromPos = getPosition(L, 3);
	Position& toPos = getPosition(L, 4);
	if (item) {
		g_actions->useItemEx(player, fromPos, toPos, 255, item, false);
		pushBoolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int32_t LuaScriptInterface::luaMonsterExecuteSpell(lua_State* L)
{
	// monster:executeSpell(spellId)
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		monster->doAttackingSpell(0, getNumber<uint32_t>(L, 2));
	}
	else {
		lua_pushnil(L);
	}
